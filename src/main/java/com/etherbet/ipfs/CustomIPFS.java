...
public class CustomIPFS {
    ...
    public CustomIPFS(String host, int port) {
        this(host, port, "/api/v0/", false);
    }

    public CustomIPFS(String multiaddr) {
        this(new MultiAddress(multiaddr));
    }

    public CustomIPFS(MultiAddress addr) {
        this(addr.getHost(), addr.getTCPPort(), "/api/v0/", detectSSL(addr));
    }

    public CustomIPFS(String host, int port, String version, boolean ssl) {
        this.host = host;
        this.port = port;
    ...
    private InputStream retrieveStream(String path) throws IOException {
        ...
        return CustomIPFS.getStream(target);
    }
